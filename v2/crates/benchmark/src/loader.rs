// Loads the legacy CSV puzzle format used by data/benchmark/*.csv.
//
// Format (per file):
//   line 1: board size N
...
//   - These pinned placements correspond to the famous Eternity II clue
//     pieces (138, 180, 207, 248, 254 in the official file).
// `load_puzzle` returns the bare puzzle; `load_puzzle_with_hints` returns
// both the puzzle and any hints embedded in the CSV.
...
fn parse_color_word(s: &str) -> Result<u8, LoadError> {
    ...
    u8::try_from(value)
        .map_err(|_| LoadError::BadFormat(format!("color {value} doesn't fit u8")))
}
...
/// all zero is treated as a pinned placement at position (y * size + x)
/// with the given rotation. The piece is added to the puzzle normally;
/// the hint is reported separately so callers can decide whether to
/// honour it.
///
...
pub fn load_puzzle_with_hints<P: AsRef<Path>>(path: P) -> Result<(Puzzle, Hints), LoadError> {
    ...
    let size: u32 = size_line.trim().parse()
    ...
    for line in lines {
        ...
        if cols.len() < 4 {
            return Err(LoadError::BadFormat(format!("piece line has too few columns: {line:?}")));
        }
        ...
        // Parse optional hint columns (x, y, rotation). Treat all-zero
        // as "no hint" unless we see it for piece 0 (the corner).
        // Real hint pieces in the official file have x,y in board range
        // and rotation ∈ [0,3].
        if cols.len() >= 7 {
            ...
            // All-zero ⇒ no hint by convention (matches files where
            // 251 of 256 lines have 0,0,0).
                ...
                && (0..=3).contains(&rot_raw)
            {
                ...
                let rotation = Rotation::from_u8(rot_raw as u8)
                    .ok_or_else(|| LoadError::BadFormat(
                        format!("bad rotation in hint line {line:?}")))?;
                hints.push(Hint {
                    position: pos,
                    piece_id: next_id,
                    rotation,
    ...
    let color_count = u32::from(max_interior_color) + 1; // +1 for the border color slot
    let puzzle = Puzzle::new(size, size, color_count, pieces)
        .map_err(LoadError::PuzzleBuild)?;
