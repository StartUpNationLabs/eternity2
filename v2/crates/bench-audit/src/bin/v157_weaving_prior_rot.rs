// V157 WEAVING-PRIOR-ROT — beam search with rotation-aware empirical prior.
//
// Fork of v155_weaving_prior. The prior is now 3D:
//   prior[piece_id * N_ROT + rotation][position] = count
// This is more discriminative than the 2D (piece_id, position) prior
// because it distinguishes WHICH rotation of a piece is common at a
// given cell. Tighter bias → potentially higher score lift.
...
fn main() {
        ...
        for state in &beam {
            for &pr_idx in &by_class[class_idx] {
                ...
                let prior_inc = match &prior {
                    Some(p) => p[pr.piece_id as usize * N_ROT + pr.rot as usize][pos],
                    None => 0,
                };
    ...
    if let Some(b) = best {
        println!("{{\"profile\":\"weaving_prior_rot_v157\",\"scan\":\"{scan_mode}\",\"beam_width\":{beam_width},\"prior\":{},\"elapsed_ms\":{},\"beam_final\":{},\"best_score\":{}}}",
        ...
        println!("{{\"profile\":\"weaving_prior_rot_v157\",\"elapsed_ms\":{},\"best_score\":0,\"error\":\"empty_beam\"}}",
            elapsed.as_millis());
    }
}
