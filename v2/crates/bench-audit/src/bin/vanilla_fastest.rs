// Vol-32+ FASTEST variant — allows unsafe for hot-loop bounds elision.
// CLAUDE.md normally forbids unsafe; user explicitly authorized for this bin.
//
// Optimisations on top of vanilla_fast:
//   - `unsafe { *get_unchecked(i) }` on bucket_data, chosen, used in hot loop
//   - All other tricks (packed entries, flat buckets, bucket sort) from vanilla_fast
//
// Goal: 150M+ pp/s single-thread, 1B+ aggregate on 8 cores.
...
fn main() {
    ...
    let thread_results: Vec<ThreadResult> = (0..threads).into_par_iter().map(|thread_id| {
        ...
        // SAFETY for unsafe blocks below:
        //  - `d` (= depth) is always in 0..N_POS during search; loop terminates if depth==N_POS
        //    via solve path or backtracks to 0 via the if-depth==0 break.
        //  - `pos` (= d) is in 0..N_POS; pos-N is in 0..N_POS-N when pos >= N; pos-1 valid when pos%N != 0.
        //  - `bucket_starts` has length total_keys + 1 (N_POS * NW_KEYS + 1).
        //  - `bucket_data` length = total_entries + hint count.
        //  - `chosen`, `frame_cursor`, `bucket_start_at_depth`, `bucket_end_at_depth`, `depth_placements`: N_POS elements.
        //  - `used`: N_PIECES = 256 elements, indexed by entry_piece_id which is 9 bits.
        //  - `piece_rots`: N_PIECES * N_ROT = 1024 elements.
        macro_rules! enter_fresh {
            ($d:expr) => {{
                ...
                let hint_idx = unsafe { *hint_entry_idx.get_unchecked(pos) };
                if hint_idx != u32::MAX {
                    let hint_entry = unsafe { *my_bucket_data.get_unchecked(hint_idx as usize) };
                    let hint_pid = entry_piece_id(hint_entry);
                    let hint_rot = entry_rot(hint_entry);
                    let pr = unsafe { piece_rots.get_unchecked(hint_pid as usize * N_ROT + hint_rot as usize) };
                    if pr.n == n_color && pr.w == w_color {
                        unsafe {
                            *bucket_start_at_depth.get_unchecked_mut(d) = hint_idx;
                            *bucket_end_at_depth.get_unchecked_mut(d) = hint_idx + 1;
                            *frame_cursor.get_unchecked_mut(d) = hint_idx;
                        }
                    } else {
                        unsafe {
                            *bucket_start_at_depth.get_unchecked_mut(d) = 0;
                            *bucket_end_at_depth.get_unchecked_mut(d) = 0;
                            *frame_cursor.get_unchecked_mut(d) = 0;
                        }
                    }
                } else {
                    let key = (n_color as usize) * N_COLORS + (w_color as usize);
                    let bkt_idx = pos * NW_KEYS + key;
                    let s = unsafe { *bucket_starts.get_unchecked(bkt_idx) };
                    let e = unsafe { *bucket_starts.get_unchecked(bkt_idx + 1) };
                    unsafe {
                        *bucket_start_at_depth.get_unchecked_mut(d) = s;
                        *bucket_end_at_depth.get_unchecked_mut(d) = e;
                        *frame_cursor.get_unchecked_mut(d) = s;
                    }
                }
            }};
        }
        ...
        'outer: loop {
            ...
            let end = unsafe { *bucket_end_at_depth.get_unchecked(depth) };
            let mut cur = unsafe { *frame_cursor.get_unchecked(depth) };
            let mut found = u32::MAX;
            while cur < end {
                let entry = unsafe { *my_bucket_data.get_unchecked(cur as usize) };
                let pid = (entry >> 12) as usize;
                if unsafe { !*used.get_unchecked(pid) } {
                    found = entry;
                    break;
                }
                cur += 1;
            ...
            if found != u32::MAX {
                unsafe {
                    ...
                    *used.get_unchecked_mut(pid) = true;
                    *frame_cursor.get_unchecked_mut(depth) = cur;
                }
                total_placements += 1;
                unsafe { *depth_placements.get_unchecked_mut(depth) += 1; }
                depth += 1;
                ...
                if depth == N_POS {
                    ...
                    depth -= 1;
                    unsafe {
                        let pid = (*chosen.get_unchecked(depth) >> 12) as usize;
                        *used.get_unchecked_mut(pid) = false;
                        *frame_cursor.get_unchecked_mut(depth) += 1;
                    }
                    continue;
                }
                enter_fresh!(depth);
