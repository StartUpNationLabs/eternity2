...
fn main() {
    ...
    // V189 CORTEZ — target corner-perm (piece IDs at positions 0, 15, 240, 255).
    let mut target_cp: Option<[u16; 4]> = None;
    ...
    while i < raw.len() {
        match raw[i].as_str() {
            ...
            "--target-cp" => {
                ...
                assert_eq!(parts.len(), 4, "--target-cp expects 4 piece IDs comma-separated, e.g. 0,2,3,1");
                let cp: [u16; 4] = [
                    parts[0].parse().expect("cp[0]"),
                    parts[1].parse().expect("cp[1]"),
                    parts[2].parse().expect("cp[2]"),
                    parts[3].parse().expect("cp[3]"),
                ];
                target_cp = Some(cp);
                i += 2;
            }
    ...
    for depth in 0..N_POS {
        ...
        for state in &beam {
            for &pr_idx in &by_class[class_idx] {
                ...
                if !bc[2] && pr.s == BORDER { continue; }
                if !bc[3] && pr.w == BORDER { continue; }
                // V189 CORTEZ: pin corner piece IDs to target_cp at positions 0, 15, 240, 255.
                if let Some(cp) = target_cp {
                    ...
                    if let Some(req_pid) = required {
                        if pr.piece_id != req_pid { continue; }
                    }
                }
