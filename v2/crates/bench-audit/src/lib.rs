...
/// On canonical 16×16 this is 480. Used as denominator when scoring
/// partial boards in the score / matches metric.
#[must_use]
...
pub fn render_board(puzzle: &Puzzle, board: &Board) -> String {
    ...
    for y in 0..puzzle.height {
        for x in 0..puzzle.width {
            ...
            match board.get(pos) {
                Some((pid, rot)) => out.push_str(&format!("{:>4}:{} ", u32::from(pid), rot.as_u8())),
                None => out.push_str("----:- "),
            }
        }
        out.push('\n');
