...
pub struct DumpedBoard {
    ...
    pub score: u32,
    pub total_edges: u32,
    // One slot per cell, row-major. None for empty cells; otherwise [piece_id, rotation].
    pub cells: Vec<Option<[u32; 2]>>,
}

impl DumpedBoard {
    pub fn from_board(board: &Board, seed: u64, score: u32, total_edges: u32) -> Self {
        let cells = board.cells().iter().map(|c| {
            c.map(|(pid, rot)| [u32::from(pid), u32::from(rot.as_u8())])
        }).collect();
        Self {
            width: board.width(),
            height: board.height(),
