// Vol-125 T10 v6 — adds Regin domain filtering on top of v5 HK matching.
//
...
struct State {
    ...
    /// Tarjan SCC workspace (size 512 = 256 pieces + 256 cell-slots).
    /// Nodes 0..256 = pieces, 256..512 = cell-slots.
    scc_index: Box<[i32; 512]>,
    scc_lowlink: Box<[i32; 512]>,
    scc_onstack: Box<[bool; 512]>,
    scc_id: Box<[i32; 512]>,
    scc_stack: Vec<u16>,
}
...
impl State {
    fn load(in_dir: &PathBuf) -> Self {
        ...
        State {
            ...
            scc_index: Box::new([-1; 512]),
            scc_lowlink: Box::new([0; 512]),
            scc_onstack: Box::new([false; 512]),
            scc_id: Box::new([-1; 512]),
            scc_stack: Vec::with_capacity(512),
        }
    }
    ...
    /// Régin's filter: after computing max matching, find edges NOT in any max
    /// matching. Remove them. Returns total blocks removed, or None if UNSAT.
    ///
    /// Standard algorithm (Régin 1994):
    /// 1. Compute max matching M via Hopcroft-Karp.
    /// 2. If M < 256, return None (UNSAT).
    /// 3. Build directed graph G:
    ///    - For edge (p, cs) in M: edge cs → p
    ///    - For edge (p, cs) not in M: edge p → cs
    /// 4. Compute SCCs.
    /// 5. Edge (p, cs) is NOT in any max matching iff p and cs are in different SCCs.
    /// 6. Remove blocks using those (piece, slot) pairs.
    ///
    /// For our perfect-matching case (M = 256), all vertices are matched, so
    /// step 3 is simpler (no source/sink needed).
    fn regin_filter(&mut self) -> Option<u64> {
        let m = self.hopcroft_karp();
        if m < 256 { return None; }

        // SANITY CHECK: verify matching is valid before SCC.
        for p in 0..256 {
            let cs = self.hk_match_p[p];
            assert!(cs >= 0 && cs < 256, "piece {} matched to invalid cs={}", p, cs);
            assert_eq!(self.hk_match_s[cs as usize], p as i16, "matching inconsistent at p={}, cs={}", p, cs);
        }

        // Build adjacency lists for the directed graph G.
        // Node IDs: piece p → id = p (0..256), cell-slot cs → id = 256+cs (256..512).
        let mut adj: Vec<Vec<u16>> = vec![Vec::with_capacity(8); 512];
        for p in 0..256i16 {
            let matched_cs = self.hk_match_p[p as usize];
            // Edges out of piece p:
            for &cs in &self.hk_piece_adj[p as usize] {
                if cs as i16 == matched_cs {
                    // Edge cs → p (in matching).
                    adj[256 + cs as usize].push(p as u16);
                } else {
                    // Edge p → cs (not in matching).
                    adj[p as usize].push(256 + cs);
                }
            }
        }

        // Tarjan SCC (iterative).
        for v in self.scc_index.iter_mut() { *v = -1; }
        for v in self.scc_id.iter_mut() { *v = -1; }
        for v in self.scc_onstack.iter_mut() { *v = false; }
        self.scc_stack.clear();

        let mut next_idx: i32 = 0;
        let mut next_scc: i32 = 0;
        for root in 0..512u16 {
            if self.scc_index[root as usize] >= 0 { continue; }
            // Iterative Tarjan DFS.
            // call_stack entries: (node, adj_pos)
            let mut call_stack: Vec<(u16, u16)> = vec![(root, 0)];
            self.scc_index[root as usize] = next_idx;
            self.scc_lowlink[root as usize] = next_idx;
            next_idx += 1;
            self.scc_stack.push(root);
            self.scc_onstack[root as usize] = true;
            while let Some(&(node, pos)) = call_stack.last() {
                ...
                if (pos as usize) < neighbors.len() {
                    ...
                    if self.scc_index[w as usize] == -1 {
                        ...
                        call_stack.push((w, 0));
                    } else if self.scc_onstack[w as usize] {
                        let cur = node as usize;
                        let w_idx = self.scc_index[w as usize];
                        if w_idx < self.scc_lowlink[cur] {
                            self.scc_lowlink[cur] = w_idx;
                        }
                    }
                } else {
                    // Finish node.
        ...
        for p in 0..256i16 {
            ...
            // Copy adjacency to avoid borrow conflict during iteration.
            let edges: Vec<u16> = self.hk_piece_adj[p as usize].clone();
            for cs in edges {
                ...
                if scc_p != scc_cs {
                    ...
                    let sr = (cs / 32) as u8;
                    let sc = ((cs % 32) / 4) as u8;
                    let slot = (cs % 4) as u8;
                    // Skip if cell is pinned (its block is fixed; if our piece
                    // isn't in the pinned block, slot_block_count would already be 0).
                    if self.pinned[sr as usize][sc as usize] { continue; }
                    // Iterate live blocks at this cell; find ones using piece p in slot.
                    let blocks_to_remove: Vec<u32> = {
                        let d = &self.domain[sr as usize][sc as usize];
                        let alphabet = &self.alphabets[sr as usize][sc as usize];
                        d.iter_live().iter().copied().filter(|&i| {
                            let b = alphabet[i as usize];
                            b.pieces[slot as usize] == p as u16
                        }).collect()
                    };
                    for bi in blocks_to_remove {
                        if self.remove(sr, sc, bi) {
                            total_removed += 1;
                            if self.domain[sr as usize][sc as usize].len() == 0 {
                                return None;
                            }
                        }
                    }
