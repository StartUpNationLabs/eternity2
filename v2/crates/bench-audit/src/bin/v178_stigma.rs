...
fn main() {
    ...
    for depth in 0..N_POS {
        ...
        if stochastic_temperature > 0.0 {
            ...
            for c in children.drain(..) {
                let logit = (c.score as f64 + alpha * c.prior_sum as f64
                             + pheromone_weight * c.pheromone_sum) * inv_t;
        ...
        } else if pheromone_weight > 0.0 || prior_alpha > 0.0 {
            let alpha = prior_alpha;
            let lambda = pheromone_weight;
            // f64 combined key; sort by descending.
            let mut keyed: Vec<(f64, BeamState)> = Vec::with_capacity(children.len());
            for c in children.drain(..) {
                let key = c.score as f64 + alpha * c.prior_sum as f64 + lambda * c.pheromone_sum;
                keyed.push((key, c));
            }
    ...
    if let Some(b) = best {
        println!("{{\"profile\":\"v178_stigma\",\"scan\":\"{scan_mode}\",\"beam_width\":{beam_width},\"prior\":{},\"elapsed_ms\":{},\"beam_final\":{},\"best_score\":{}}}",
        ...
        println!("{{\"profile\":\"v178_stigma\",\"elapsed_ms\":{},\"best_score\":0,\"error\":\"empty_beam\"}}",
            elapsed.as_millis());
    }
}
