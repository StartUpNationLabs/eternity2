...
### `unsat-clause-propagator` (Marie + Akos's database) — status: `unbuilt` — since: vol-32 open
capiman/e2 GitHub repo ships ~70M pre-computed unsat 2-piece clauses (Git LFS, 336 MB total, cloned at `output/capiman_e2/`). Each clause `-X -Y 0` says SAT literals X and Y (each a `(piece, position, rotation)` triple) cannot co-exist in any solution. Build a Rust propagator that loads the .cnf.gz at engine init and, on each `place_and_propagate`, looks up the placed literal in a hashmap `lit → forbidden_set` and removes those literals from remaining domains. **External knowledge encoded as engine-level constraints** — first time we use community-verified pruning. Likely largest pruning gain since gacolor + AC-3. Honest cost: 1 day build (CNF parser + propagator + index) + measurement. Vol-33+ T1 candidate.
