// Vol-35 — size-generic version of vanilla_fast. Runtime board size,
// piece count, color count read from the puzzle file.
//
// Trade-off vs vanilla_fast (16×16-hardcoded):
//   - Throughput drops ~5× on 16×16 (~13M pp/s vs ~125M for hardcoded)
//     because the inner loop can't const-propagate `pos % N`, `pos < N`.
//   - But works at 4×4, 6×6, 8×8, 10×10, 12×12, 16×16, etc.
//
// For landscape research at small sizes, this throughput is fine.
// For canonical 16×16 record-chase, use vanilla_fast (hardcoded).
//
// Run:
//   target/release/vanilla_generic --budget-ms 10000 \
//       --puzzle ../data/generated/size_8_colors_5_*.csv
...
// Vol-35 — n, n_pos, n_pieces, n_colors, nw_keys are read at runtime from
// the puzzle file. Only N_ROT (4) is compile-time constant.
const N_ROT: usize = 4;
