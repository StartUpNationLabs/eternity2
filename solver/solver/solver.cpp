...
void merge_shared_data(SharedData &global_data, std::vector<SharedData> &local_data) {
    global_data.board_count = 0;
    global_data.hash_hit_count = 0;
    global_data.pieces_placed = 0;
    global_data.redis_hash_count = 0;
    global_data.max_count = 0;
    for (const auto &local: local_data) {
        global_data.board_count += local.board_count;
        global_data.hash_hit_count += local.hash_hit_count;
        global_data.pieces_placed += local.pieces_placed;
        global_data.redis_hash_count += local.redis_hash_count;
        global_data.hashes.insert(local.hashes.begin(), local.hashes.end());
        // update the max board

        if (local.max_count > global_data.max_count) {
            global_data.max_count = local.max_count;
            global_data.max_board = local.max_board;
        }
        // update the stop flag
        global_data.stop = global_data.stop || local.stop;

    }

    // update the local data from the global data
    for (SharedData &local: local_data) {
        local.hashes = global_data.hashes;
        local.max_count = global_data.max_count;
        local.max_board = global_data.max_board;
        local.stop = global_data.stop;
    }
