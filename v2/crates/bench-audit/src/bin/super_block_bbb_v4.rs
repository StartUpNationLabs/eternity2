// Vol-125 T10 v4 — adds piece-live-count + forced-piece propagation.
//
...
struct State {
    ...
    /// piece_live_count[p] = number of currently-live piece occurrences in domains.
    /// Equal to sum of [b in alphabet[sr][sc].buf[..live] if p in b.pieces].
    piece_live_count: [u32; 256],
    trail: Vec<(u8, u8, u32)>,
    /// When True, forced-piece queue should be processed in propagation.
    forced_queue: Vec<u16>,
}
...
impl State {
    fn load(in_dir: &PathBuf) -> Self {
        ...
        // Initialize piece_live_count from piece_occ (each occurrence is alive).
        let mut piece_live_count = [0u32; 256];
        for p in 0..256 {
            piece_live_count[p] = piece_occ[p].len() as u32;
        }
        ...
        State {
            ...
            forced_queue: Vec::with_capacity(256),
        }
    }
    ...
    /// Returns true if it was actually removed. Also decrements piece_live_count
    /// for the block's 4 pieces, and enqueues forced pieces.
    #[inline]
    fn remove(&mut self, sr: u8, sc: u8, block_idx: u32) -> bool {
        ...
        if d.pos_of[block_idx as usize] < d.live {
            ...
            d.remove(block_idx);
            // Decrement piece_live_count for each of the 4 pieces.
            let b = self.alphabets[sr as usize][sc as usize][block_idx as usize];
            for &p in &b.pieces {
                self.piece_live_count[p as usize] = self.piece_live_count[p as usize].saturating_sub(1);
                // If a non-used piece's count drops to 0, that's UNSAT.
                // If it drops to 1, it's forced (only one place).
                // We enqueue and let the caller process.
                if !self.used_pieces[p as usize] && self.piece_live_count[p as usize] <= 1 {
                    self.forced_queue.push(p);
                }
            }
            true
        } else { false }
    ...
    /// Process the forced-piece queue: for any piece p with piece_live_count == 0
    /// (UNSAT) or == 1 (forced), pin its cell.
    /// Returns None on UNSAT, Some(forced_count) on success.
    fn process_forced(&mut self) -> Option<u64> {
        let mut forced_count: u64 = 0;
        // Drain the queue; processing may add more entries.
        let mut i = 0;
        while i < self.forced_queue.len() {
            let p = self.forced_queue[i] as usize;
            i += 1;
            if self.used_pieces[p] { continue; } // already pinned somewhere
            let live = self.piece_live_count[p];
            if live == 0 {
                return None; // piece has nowhere to go
            }
            if live > 1 { continue; } // not actually forced; this is a stale entry

            // Find the single live (sr, sc, block_idx) for piece p.
            // Linear scan through piece_occ[p] (typically O(2M) but rare events).
            let mut found: Option<(u8, u8, u32)> = None;
            for &(osr, osc, b_idx) in &self.piece_occ[p] {
                if self.pinned[osr as usize][osc as usize] {
                    // Pinned cell — if this is the cell with piece p, treat as found.
                    // But we shouldn't reach here for non-hint cells.
                    // For hint cells: piece is owned and not "forced" in the search sense.
                    continue;
                }
                let d = &self.domain[osr as usize][osc as usize];
                if d.pos_of[b_idx as usize] < d.live {
                    if found.is_some() {
                        // More than one occurrence found — count was stale. Skip.
                        found = None;
                        break;
                    }
                    found = Some((osr, osc, b_idx));
                }
            }
            if let Some((osr, osc, b_idx)) = found {
                // Pin (osr, osc) to b_idx, which uses piece p (and 3 others).
                forced_count += 1;
                self.pin_and_propagate_pieces(osr as usize, osc as usize, b_idx)?;
                self.boundary_propagate_from_pin(osr as usize, osc as usize, b_idx)?;
                // Don't reset i; continue draining (queue may have grown).
            }
        }
        self.forced_queue.clear();
        Some(forced_count)
    }
...
fn rewind_trail(state: &mut State, start: usize) {
    // Drain forced queue first (could contain stale entries).
    ...
    while state.trail.len() > start {
        ...
        if a == 0xFF {
            ...
            if d.live < prev_live {
                // Re-activate blocks in range [d.live, prev_live).
                // For each re-activated block, re-increment piece_live_count.
                ...
                for pos in d.live as usize..prev_live as usize {
                    ...
                    for &p in &block.pieces {
                        state.piece_live_count[p as usize] += 1;
                    }
                }
                d.live = prev_live;
            }
...
fn main() {
    ...
    eprintln!("=== Super-block BB&B v4 (incremental + forced-piece propagation + O(1) sparse-set) ===");
