...
export const settingsStatisticsState = atom({
...
export const generatedBoardsState: RecoilState<Board[]> = atom<Board[]>({
    ...
    for (let colors = minColors; colors <= maxColors; colors++) {
      for (let i = 0; i < sampleSize; i++) {
        boards.push({
          ...
          pieces: convertToPieces(
            shuffleAndRotateBoard(createBoard(size, colors))
          ).map((piece, index) => ({
            index: index,
            piece: piece,
            rotation: 0,
