...
auto handle_server_solver_request(agrpc::GrpcContext &grpc_context,
    ...
    return agrpc::register_sender_rpc_handler<SolverRPC>(
            grpc_context, service1, [&](SolverRPC &rpc, SolverRPC::Request &request) -> unifex::task<void> {
                ...
                SharedData shared_data = {max_board, static_cast<uint64_t>(max_count), mutex, hashes};
                ...
                // local shared data
                std::vector<SharedData> local_data(thread_count, shared_data);
                for (int i = 0; i < thread_count; i++) {
                    threads.emplace_back(thread_function, board, pieces, std::ref(local_data[i]));
                }
                ...
                while (true) {
                    ...
                    if (shared_data.max_count == board.size * board.size) {
                        ...
                        merge_shared_data(shared_data, local_data);
                    ...
                    spdlog::info(
                            "Writing response, boards analyzed: {}, hashes: {}, board size {}, boards per second: {}",
                            shared_data.board_count,
                            shared_data.hashes.size(),
                            board.size,
                            shared_data.board_count / seconds_since_start);
