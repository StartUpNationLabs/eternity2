...
fn main() {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--seed" => { seeds = vec![args[i + 1].parse().unwrap()]; i += 2; }
            "--seeds" => {
    ...
    eprintln!("=== Super-block BB&B v5_fast (in-process multi-seed) ===");
    eprintln!("Seeds: {:?}, max_nodes per seed: {}", seeds, max_nodes);
    ...
    // Snapshot the post-AC-3 state. We clone for each seed (so each seed
    // starts from the same baseline). Trail must be empty here.
    state.trail.clear();
    let base_state = state.clone();
    let total_dfs_t = Instant::now();
    eprintln!("\n=== Running {} seed(s) ===", seeds.len());
    for (idx, &seed) in seeds.iter().enumerate() {
        eprintln!("\n--- seed={} ({}/{}) ---", seed, idx + 1, seeds.len());
        let mut state = base_state.clone();
        let t_dfs = Instant::now();
        let mut nodes = 0u64;
        match dfs_seeded(&mut state, 0, max_nodes, &mut nodes, t_dfs, seed) {
            Some(placement) => {
                eprintln!("\n🎯 SEED={} 480 SOLUTION found after {} nodes ({:.1}s)",
                          seed, nodes, t_dfs.elapsed().as_secs_f64());
                let json = serde_json::json!({
                    ...
                    "source": "super_block_bbb_v5_fast"
                });
                let path = format!("output/vol-125/SOLUTION_480_seed{}.json", seed);
                std::fs::write(&path, serde_json::to_string_pretty(&json).unwrap()).expect("write");
                eprintln!("Saved to: {}", path);
            }
            None => {
                eprintln!("\nseed={} DFS exhausted/exceeded after {} nodes ({:.1}s) without 480.",
                          seed, nodes, t_dfs.elapsed().as_secs_f64());
            }
        }
    }
    eprintln!("\n=== ALL SEEDS DONE in {:.1}s ===",
              total_dfs_t.elapsed().as_secs_f64());
}
