...
fn main() {
    ...
    if args.len() < 2 {
        eprintln!("usage: peps_bench <puzzle.csv> [n_iter=10] [chi=0]");
        std::process::exit(1);
    }
    let path = PathBuf::from(&args[1]);
    let n_iter: usize = args.get(2).and_then(|s| s.parse().ok()).unwrap_or(10);
    let chi: usize = args.get(3).and_then(|s| s.parse().ok()).unwrap_or(0);

    let puzzle = load_puzzle(&path).expect("load");
    let ctx = PepsContext::new(Arc::new(puzzle));
    println!("Puzzle: size={} K={} n_pieces={} chi={}", ctx.size, ctx.k_colors, ctx.n_pieces, chi);

    let mu = vec![0.0; ctx.n_pieces];
    let pinned: Vec<Option<PieceRot>> = vec![None; ctx.size * ctx.size];

    // Warmup
    let log_z_warm = log_z_lagrangian(&ctx, &mu, &pinned, chi);
    ...
    let mut last = 0.0;
    for _ in 0..n_iter {
        last = log_z_lagrangian(&ctx, &mu, &pinned, chi);
    }
    let dt = t0.elapsed();
