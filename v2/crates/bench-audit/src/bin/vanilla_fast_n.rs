// Vol-35 — runtime-N variant of vanilla_fast.
//
// Identical algorithm to vanilla_fast.rs (the hyper-optimized N=16
// hardcoded version). Differences:
//   - N, n_pos, n_pieces, n_colors, nw_keys are determined from the
//     loaded puzzle at runtime, not const-baked.
//   - Stack arrays [T; n_pos] become Vec<T> with capacity from puzzle.
//   - Helper functions take n/n_pos as parameters.
//
// Expected throughput: 80-100M placements/sec at 16×16 (vs 125M for
// the hardcoded vanilla_fast — ~10-20% slower due to Vec<T> bounds
// checks and missed const-prop). For smaller sizes the throughput
// is similar in placements/sec but absolute solve time is faster.
//
// USE THIS for non-16×16 puzzles (generated 4×4, 6×6, 8×8, 10×10,
// 12×12) for landscape mapping. Use vanilla_fast for the canonical
// 16×16 hot path.
//
// Run:
//   target/release/vanilla_fast_n --budget-ms 10000 --puzzle <CSV>
...
// piece_id in 0..512 → 9 bits. rot 0..3 → 2 bits. S, E in 0..31 → 5 bits each.
// At canonical (256 pieces × 23 colors) and smaller sizes, fits comfortably.
#[inline(always)]
...
#[inline(always)]
fn need_north_border(pos: usize, n: usize) -> bool { pos < n }
#[inline(always)]
fn need_west_border(pos: usize, n: usize) -> bool { pos % n == 0 }
#[inline(always)]
fn need_south_border(pos: usize, n: usize, n_pos: usize) -> bool { pos >= n_pos - n }
#[inline(always)]
fn need_east_border(pos: usize, n: usize) -> bool { pos % n == n - 1 }
...
fn main() {
    ...
    let n: usize = puzzle.width as usize;
    assert_eq!(puzzle.height as usize, n, "puzzle must be square");
    let n_pos: usize = n * n;
    let n_pieces: usize = puzzle.pieces().len();
    let n_colors: usize = puzzle.color_count as usize;
    let nw_keys: usize = n_colors * n_colors;
    eprintln!(
        "[init] sizes: n={n}, n_pos={n_pos}, n_pieces={n_pieces}, n_colors={n_colors}"
    );
    assert!(n_pieces <= 512, "piece_id needs ≤ 9 bits");
    assert!(n_colors <= 32, "S/E colors need ≤ 5 bits");
    ...
    for pos in 0..n_pos {
        let need_n_border = need_north_border(pos, n);
        let need_e_border = need_east_border(pos, n);
        let need_s_border = need_south_border(pos, n, n_pos);
        let need_w_border = need_west_border(pos, n);
    ...
    let mut pid_global_count: Vec<u32> = vec![0u32; n_pieces];
    ...
    let max_internal_edges = 2 * n * n - 2 * n; // 2*W*H - W - H for square
    eprintln!("[best-partial] depth={} matched_internal={}/{} matched_border={} matched_total={}",
        best_depth, matched_internal, max_internal_edges, matched_border, best_score);
